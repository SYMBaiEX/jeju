...
import { MomentumStrategy, type MomentumConfig } from './momentum-strategy';
import { MeanReversionStrategy, type MeanReversionConfig } from './mean-reversion-strategy';
import { VolatilityStrategy, type VolatilityConfig } from './volatility-strategy';
...
export interface CompositeConfig {
  ...
  conflictResolution: 'average' | 'strongest' | 'momentum-first' | 'volatility-first';
...
export class CompositeStrategy extends BaseTFMMStrategy {
  ...
  constructor(
    ...
    this.meanReversionStrategy = new MeanReversionStrategy(oracle, config.meanReversionConfig);
    this.volatilityStrategy = new VolatilityStrategy(oracle, config.volatilityConfig);
  }
  ...
  async calculateWeights(ctx: StrategyContext): Promise<WeightCalculation> {
    ...
    if (this.config.enableRegimeDetection) {
      ...
      strategyWeights = this.adjustWeightsForRegime(strategyWeights, this.currentRegime);
    }
    ...
    const combinedSignals = this.combineSignals(
      ...
      [
        ...
        { signals: meanRevResult.signals, weight: strategyWeights.meanReversion },
    ...
    const combinedWeights = this.combineWeights(
      ...
      [strategyWeights.momentum, strategyWeights.meanReversion, strategyWeights.volatility]
    );

    // Normalize and apply guard rails
    const normalizedWeights = this.normalizeWeights(combinedWeights);
    const safeWeights = this.applyGuardRails(ctx.currentWeights, normalizedWeights, ctx.riskParams);

    // Calculate combined confidence
    const confidence = this.calculateCombinedConfidence(
      [momentumResult.confidence, meanRevResult.confidence, volResult.confidence],
      [strategyWeights.momentum, strategyWeights.meanReversion, strategyWeights.volatility]
    );
  ...
  private detectRegime(
    ...
    const momentumStrength = momentumResult.signals
    ...
    const volStrength = volResult.signals
      .filter(s => s.token !== 'REGIME')
      .reduce((sum, s) => sum + Math.abs(s.signal) * s.strength, 0) /
  ...
  private adjustWeightsForRegime(
    baseWeights: { momentum: number; meanReversion: number; volatility: number },
    regime: MarketRegime
   ...
   */
  override updatePriceHistory(prices: import('../../types').OraclePrice[]): void {
