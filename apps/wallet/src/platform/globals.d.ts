...
interface SafariMessageUserInfo {
  [key: string]: string | number | boolean | null | SafariMessageUserInfo | SafariMessageUserInfo[];
}

/** Tauri API interface (minimal type for detection) */
interface TauriAPI {
  invoke: (cmd: string, args?: Record<string, string | number | boolean>) => Promise<string | number | boolean | null>;
}
...
declare global {
  ...
  const safari: {
    extension?: {
      dispatchMessage?: (name: string, userInfo?: SafariMessageUserInfo) => void;
      getBaseURI?: () => string;
    };
    application?: {
      activeBrowserWindow?: {
  ...
  interface Window {
    ...
    __SEND_POPUP_RESPONSE__?: (requestId: string, approved: boolean, data?: PopupResponseData) => void;
  }
