...
export function parseAndValidateParam(
  ...
  expectDefined(param, `${context ?? 'Route parameter'} '${paramName}' is required`);
 ...
 */
export function parseProposalId(value: string | undefined, context: string): `0x${string}` {
  expectDefined(value, `${context}: proposalId is required`);
  expectTrue(typeof value === 'string', `${context}: proposalId must be a string`);
  expectTrue(isHex(value) && value.length === 66, `${context}: Invalid proposalId format: ${value}`);
...
export function parseInteger(
  ...
  if (typeof value === 'number') {
    ...
    if (min !== undefined) expectTrue(value >= min, `${context}: must be >= ${min}`);
    if (max !== undefined) expectTrue(value <= max, `${context}: must be <= ${max}`);
    return value;
  }
  ...
  if (min !== undefined) expectTrue(parsed >= min, `${context}: must be >= ${min}`);
  if (max !== undefined) expectTrue(parsed <= max, `${context}: must be <= ${max}`);
  return parsed;
}
...
export function validateEnum<T extends string>(
  ...
  expectDefined(value, `${context}: value is required`);
  expectTrue(enumValues.includes(value as T), `${context}: Invalid value '${value}'. Must be one of: ${enumValues.join(', ')}`);
  return value as T;
}
...
export function expectStringLength(
  ...
  expectTrue(value.length >= min, `${context}: must be at least ${min} characters`);
  if (max !== undefined) {
    expectTrue(value.length <= max, `${context}: must be at most ${max} characters`);
  }
  return value;
}
...
export function errorResponse(c: Context, message: string, status: number = 400) {
  return c.json({ error: message }, status as 200 | 201 | 400 | 401 | 403 | 404 | 500 | 502 | 503);
}
...
export function successResponse<T>(c: Context, data: T, status: number = 200) {
  return c.json(data as T & (Record<string, unknown> | unknown[]), status as 200 | 201 | 400 | 401 | 403 | 404 | 500 | 502 | 503);
}
