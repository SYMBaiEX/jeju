 ...
 * 
 * Run with: bun test tests/npm.test.ts
...
describe.skipIf(SKIP)('NPM Registry', () => {
  describe('Health Check', () => {
    test('GET /npm/health should return healthy', async () => {
      const res = await app.request('/npm/health');
      expect(res.status).toBe(200);

      const body = await res.json();
      expect(body.service).toBe('dws-npm');
  ...
  describe('NPM Compatibility Endpoints', () => {
    ...
    describe('Whoami', () => {
      test('GET /-/whoami without auth should return 401', async () => {
        const res = await app.request('/npm/-/whoami');
    ...
    describe('User Authentication', () => {
      test('PUT /-/user/org.couchdb.user:test should return token', async () => {
        const res = await app.request('/npm/-/user/org.couchdb.user:test', {
          method: 'PUT',
    ...
    describe('Token Deletion', () => {
      test('DELETE /-/user/token/:token should succeed', async () => {
        const res = await app.request('/npm/-/user/token/test-token', {
          method: 'DELETE',
        });
  ...
  describe('Package Publishing', () => {
    test('PUT /:package without auth should return 401', async () => {
      ...
      const res = await app.request('/npm/test-package', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          _id: 'test-package',
          ...
          versions: {
            '1.0.0': {
              ...
              description: 'Test package',
              dist: {
                tarball: 'http://localhost:4030/npm/test-package/-/test-package-1.0.0.tgz',
                shasum: 'abc123',
              },
            },
          },
  ...
  describe('Package Search', () => {
    test('GET /-/v1/search should return search results', async () => {
      const res = await app.request('/npm/-/v1/search?text=test');
    ...
    test('GET /-/v1/search with size limit should respect limit', async () => {
      const res = await app.request('/npm/-/v1/search?text=test&size=5');
      expect([200, 500]).toContain(res.status);
    ...
    test('GET /-/v1/search with from offset should paginate', async () => {
      const res = await app.request('/npm/-/v1/search?text=test&from=10&size=5');
      expect([200, 500]).toContain(res.status);
    ...
    test('GET /-/v1/search without text should return all', async () => {
      const res = await app.request('/npm/-/v1/search');
      expect([200, 500]).toContain(res.status);
    });
  });
  ...
  describe('Tarball Download', () => {
    test('GET /:package/-/:tarball with invalid version should return 400', async () => {
      const res = await app.request('/npm/test-package/-/test-package-invalid.tgz');
      expect(res.status).toBe(400);
    });

    test('GET /:package/-/:tarball for non-existent package should return 404', async () => {
      const res = await app.request('/npm/nonexistent-pkg/-/nonexistent-pkg-1.0.0.tgz');
      expect([404, 500]).toContain(res.status);
    });

    test('GET /:package/-/:tarball should handle scoped package names', async () => {
      const res = await app.request('/npm/@scope%2Fpackage/-/package-1.0.0.tgz');
      expect([404, 500]).toContain(res.status);
    });
  });
});
...
describe('NPM Edge Cases', () => {
  describe('Package Name Validation', () => {
    test('should handle package names with hyphens', async () => {
      const res = await app.request('/npm/my-awesome-package');
      expect([404, 500]).toContain(res.status);
    });

    test('should handle package names with underscores', async () => {
      const res = await app.request('/npm/my_package_name');
      expect([404, 500]).toContain(res.status);
    });

    test('should handle package names with numbers', async () => {
      const res = await app.request('/npm/package123');
      expect([404, 500]).toContain(res.status);
    });

    test('should handle very long package names', async () => {
      const longName = 'a'.repeat(200);
      const res = await app.request(`/npm/${longName}`);
      expect([404, 500]).toContain(res.status);
    });
  });
  ...
  describe('Version String Handling', () => {
    test('should handle semver versions', async () => {
      const res = await app.request('/npm/test-pkg/1.2.3');
      expect([404, 500]).toContain(res.status);
    });

    test('should handle prerelease versions', async () => {
      const res = await app.request('/npm/test-pkg/1.0.0-alpha.1');
      expect([404, 500]).toContain(res.status);
    });

    test('should handle build metadata versions', async () => {
      const res = await app.request('/npm/test-pkg/1.0.0+build.123');
      expect([404, 500]).toContain(res.status);
    });
  });
  ...
  describe('Scoped Packages', () => {
    test('should handle scoped package with encoded slash', async () => {
      const res = await app.request('/npm/@myorg%2Fmypackage');
      expect([404, 500]).toContain(res.status);
    });
  });
});

describe('NPM Server Integration', () => {
  test('DWS health should include npm service', async () => {
    const res = await app.request('/health');
    expect(res.status).toBe(200);

    const body = await res.json();
    expect(body.services.npm).toBeDefined();
    expect(body.services.npm.status).toBe('healthy');
  });

  test('DWS root should list npm endpoint', async () => {
    const res = await app.request('/');
    expect(res.status).toBe(200);

    const body = await res.json();
    expect(body.services).toContain('npm');
    expect(body.endpoints.npm).toBe('/npm/*');
  });
});
