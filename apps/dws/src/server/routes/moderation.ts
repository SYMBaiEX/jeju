...
import { z } from 'zod';
import { createPublicClient, createWalletClient, http, type Address, type Hex, parseAbiItem } from 'viem';
...
const getConfig = (): ModerationConfig => ({
  ...
  moderationMarketplaceAddress: (process.env.MODERATION_MARKETPLACE_ADDRESS ?? '0x0') as Address,
...
const BAN_MANAGER_ABI = [
  ...
  parseAbiItem('function applyAddressBan(address target, bytes32 caseId, string reason)'),
  parseAbiItem('function addressBans(address) view returns (bool isBanned, uint8 banType, uint256 bannedAt, uint256 expiresAt, string reason, bytes32 proposalId, address reporter, bytes32 caseId)'),
] as const;

const MODERATION_MARKETPLACE_ABI = [
  parseAbiItem('function reportAndBan(address target, string reason) returns (bytes32)'),
  parseAbiItem('function createBanCase(address target, string evidence, uint256 stake) payable returns (bytes32)'),
...
export function createModerationRouter(): Hono {
  ...
  app.post('/ban', async (c) => {
    ...
    if (isBanned) {
      return c.json({ success: true, alreadyBanned: true, message: 'Target is already banned' });
    }
    ...
    if (request.severity === 'critical' && request.autoban) {
      try {
        ...
        console.log(`[Moderation] Immediate ban executed for ${target}: ${hash}`);
    ...
    const queueItem: QueuedAction = {
      ...
      priority: request.severity === 'critical' ? 'urgent' : 
               request.severity === 'high' ? 'high' : 'normal',
      createdAt: Date.now(),
      attempts: 0,
  ...
  app.post('/submit-review', async (c) => {
    ...
    if (request.review.recommendation === 'ban' && 
      ...
      try {
        ...
        console.log(`[Moderation] Auto-ban from review executed for ${target}: ${hash}`);
  ...
  app.get('/status/:address', async (c) => {
    ...
    const statusNames = ['none', 'on_notice', 'challenged', 'banned', 'cleared', 'appealing'];
  ...
  app.post('/appeal', async (c) => {
    ...
    const queueItem: QueuedAction = {
      ...
      createdAt: Date.now(),
      attempts: 0,
      data: { caseId: request.caseId, evidence: request.evidence, stakeAmount: request.stakeAmount },
    };
