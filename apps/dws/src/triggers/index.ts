...
app.post('/triggers', async (c) => {
  const body = await c.req.json<Omit<Trigger, 'id' | 'executions' | 'lastRun' | 'lastStatus'>>();
  ...
  const trigger: Trigger = { 
    ...
    ...body, 
    executions: [],
    nextRun: body.type === 'cron' ? calculateNextCronRun(body.config.cron) : undefined,
  };
...
app.put('/triggers/:id', async (c) => {
  ...
  const updated: Trigger = { ...existing, ...body, id, executions: existing.executions };
...
async function executeTrigger(trigger: Trigger): Promise<TriggerExecution> {
  ...
  const response = await fetch(trigger.target, {
    ...
    body: trigger.method === 'GET' ? undefined : (trigger.body || JSON.stringify({ triggerId: trigger.id, timestamp: Date.now() })),
...
function setupCronTrigger(trigger: Trigger): void {
  ...
  if (intervalMs <= 0) {
    console.log(`[Triggers] Invalid cron expression for trigger ${trigger.id}: ${trigger.config.cron}`);
    return;
  }
  ...
  console.log(`[Triggers] Set up cron trigger ${trigger.id} with interval ${intervalMs}ms`);
}
...
function parseCronToInterval(cron: string): number {
  ...
  if (minutes.startsWith('*/')) {
    const n = parseInt(minutes.slice(2));
    if (!isNaN(n) && n > 0) {
      return n * 60 * 1000;
    }
  }
