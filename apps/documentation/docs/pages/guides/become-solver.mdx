...
:::note[Localnet Setup]
The SolverRegistry is not deployed by default on localnet. To test solver functionality locally, first deploy the OIF (Omni Intent Framework) contracts:

```bash
cd packages/contracts
forge script script/DeployOIF.s.sol:DeployOIF --rpc-url http://127.0.0.1:6546 --broadcast
```

Contract addresses will be output and saved to `deployments/localnet.json`.
:::
