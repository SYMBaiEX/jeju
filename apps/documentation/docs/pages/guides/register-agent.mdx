...
The IdentityRegistry supports three registration methods:

```bash
# Simple registration (creates empty identity)
cast send $IDENTITY_REGISTRY "register()" \
  --rpc-url $RPC_URL \
  --private-key $PK

# Registration with metadata URI
cast send $IDENTITY_REGISTRY "register(string)" \
  "ipfs://Qm..." \
  --rpc-url $RPC_URL \
  --private-key $PK

# Registration with metadata URI and key-value pairs
# MetadataEntry[] is an array of {key: string, value: bytes}
cast send $IDENTITY_REGISTRY \
  "register(string,(string,bytes)[])" \
  "ipfs://Qm..." \
  "[(\"a2aEndpoint\",\"0x$(echo -n 'https://myagent.com/a2a' | xxd -p)\"),(\"mcpEndpoint\",\"0x$(echo -n 'https://myagent.com/mcp' | xxd -p)\")]" \
  --rpc-url $RPC_URL \
  --private-key $PK
```
...
// With metadata URI and entries
const txWithMetadata = await client.writeContract({
  address: registry,
  abi: IdentityRegistryAbi,
  functionName: 'register',
  args: [
    'ipfs://Qm...',
    [
      { key: 'a2aEndpoint', value: toHex('https://myagent.com/a2a') },
      { key: 'mcpEndpoint', value: toHex('https://myagent.com/mcp') },
    ],
  ],
});
```
